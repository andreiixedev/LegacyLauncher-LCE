...
const DEFAULT_REPO = "andreiixedev/Drivers-and-MiscFiles";
const DEFAULT_EXEC = "Minecraft.Client.exe";
const TARGET_FILE = "ReleaseMCLCE.7z";
...
async function initTheme() {
    const savedTheme = await Store.get('legacy_theme_mode', 'light');
    setTheme(savedTheme);
    ...
    if (toggleBtn) {
        toggleBtn.addEventListener('click', async () => {
            ...
            setTheme(newTheme);
            await Store.set('legacy_theme_mode', newTheme);
        });
    }
}
...
async function migrateLegacyConfig() {
    ...
    if (!hasInstances) {
        ...
        instances = hasInstances.map(inst => ({ ...inst, repo: DEFAULT_REPO, snapshots: inst.snapshots || [] }));
...
function syncRepoPresetFromInput() {
    ...
    presetSelect.value = DEFAULT_REPO;
}
...
function getTargetRelease() {
    return releasesData.find(rel => rel.tag_name === TARGET_RELEASE_TAG);
}

function getTargetAsset(release) {
    if (!release || !Array.isArray(release.assets)) return null;
    return release.assets.find(a => a.name === TARGET_FILE);
}

function extractArchive(archivePath, extractDir) {
    if (archivePath.toLowerCase().endsWith('.zip')) {
        return extractZip(archivePath, { dir: extractDir });
    }
    ...
    if (archivePath.toLowerCase().endsWith('.7z')) {
        return new Promise((resolve, reject) => {
            const stream = Seven.extractFull(archivePath, extractDir, {
                ...
                password: TARGET_ARCHIVE_PASSWORD,
                overwrite: 'a'
            });
    ...
    return Promise.reject(new Error('Unsupported archive type: ' + path.extname(archivePath)));
}
...
async function renderInstancesList() {
    ...
    if (instances.length === 0) {
        container.innerHTML = '<div class="text-center text-muted py-4">No instances found.</div>';
        return;
    }
...
async function renderGallery() {
    ...
    try {
        ...
        if (screenshots.length === 0) {
            container.innerHTML = `
                <div class="text-center text-muted py-4">
                    ...
                    <p style="font-size: 11px; margin-top: 8px;">PRESS F2 TO TAKE ONE!</p>
                </div>
            `;
            return;
        }
        ...
        container.innerHTML = '<div class="text-center text-muted py-4" style="color: #ff5555;">ERROR LOADING GALLERY</div>';
    }
}
...
function populateUpdatesSidebar() {
    ...
    commitsData.slice(0, 30).forEach((c) => {
        const item = document.createElement('div');
        ...
        item.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                <span class="commit-sha" style="background: var(--accent); color: white; padding: 2px 8px; border-radius: 16px; font-size: 10px;">#${shortSha}</span>
                <span style="font-size: 10px; color: var(--text-muted);">${date}</span>
            </div>
            <div style="font-weight: bold; color: var(--text-primary); margin-bottom: 6px; font-size: 12px;">${escapeHtml(title)}</div>
            ${body ? `<div style="font-size: 11px; color: var(--text-secondary); padding-left: 6px; border-left: 2px solid var(--accent); margin-top: 4px;">${escapeHtml(body.substring(0, 150))}${body.length > 150 ? '...' : ''}</div>` : ''}
        `;
...
async function launchGame() {
    ...
    const release = getTargetRelease() || releasesData[currentReleaseIndex];
    if (!release || release.tag_name !== TARGET_RELEASE_TAG) {
        showToast("Release tag missing: " + TARGET_RELEASE_TAG);
        return;
    }
    const asset = getTargetAsset(release);
    if (!asset) {
        showToast("Archive asset missing: " + TARGET_FILE);
        return;
    }
...
async function checkForUpdatesManual() {
    ...
    if (rel.tag_name !== TARGET_RELEASE_TAG) {
        showToast("Release tag missing: " + TARGET_RELEASE_TAG);
        return;
    }
    const asset = getTargetAsset(rel);
    if (!asset) {
        showToast("Archive asset missing: " + TARGET_FILE);
        return;
    }
...
async function handleElectronFlow(url) {
    try {
        ...
        const archivePath = path.join(parentDir, TARGET_FILE);
...
async function loadServers() {
    ...
    if (!container) return;
    container.innerHTML = '';
    if (!fs.existsSync(filePath)) { container.innerHTML = '<div class="text-center text-muted py-4">No servers added yet.</div>'; return; }
    try {
        ...
        if (servers.length === 0) { container.innerHTML = '<div class="text-center text-muted py-4">No servers added yet.</div>'; return; }
...
async function saveOptions() {
    ...
    currentInstance.repo = DEFAULT_REPO;
...
async function promptLauncherUpdate(version, changelog) {
    return new Promise((resolve) => {
        ...
        const modalText = document.getElementById('update-modal-text');
        if (modalText) {
            modalText.innerHTML = `<span style="color: var(--accent); font-weight: bold;">NEW UPDATE: v${version}</span><br><div style="font-size: 12px; max-height: 150px; overflow-y: auto; margin-top: 8px;">${changelog || "No changelog provided."}</div>`;
...
async function renderSnapshotsList() {
    ...
    if (!inst || !inst.snapshots || inst.snapshots.length === 0) {
        container.innerHTML = '<div class="text-center text-muted py-4">No snapshots found.</div>';
        return;
    }
...
window.onload = async () => {
    try {
        ...
        } else if (musicBtnHeader) {
            musicBtnHeader.classList.add('muted');
        }
...
window.openScreenshotsGallery = openScreenshotsGallery;
